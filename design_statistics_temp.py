...
class Nussinov(object):
    ...
    def is_base_pair(self, i, j):
        return self.data[i] + self.data[j] in ('AT','TA','GC','CG')
...
def main():
    ...
    seq_name = 'p8064'
    args = parser.parse_args([file_full_path_and_name,seq_name])
    ...
    #%% compute seocndary structure metric for each balck staple
    ...
    for strand in dna_structure.strands:
        if strand.icolor==reportcolor:
            cur_sequence = []
            for domain in strand.domain_list:
                cur_sequence.append(domain.sequence.replace('N', ''))
            reporter_sequences.append(''.join(cur_sequence))    
